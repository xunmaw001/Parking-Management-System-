...
//                            yonghuEntity.setShifouhuiyuanTypes(Integer.valueOf(data.get(0)));   //是否会员 要改的
//                            yonghuEntity.setHuiyuanTime(new Date(data.get(0)));          //会员截止日期 要改的
...
//    	ValidatorUtils.validateEntity(user);
     ...
     * 重置密码
     */
    @GetMapping(value = "/xufei")
    public R xufei(Integer  id){

        YonghuEntity yonghuEntity = yonghuService.selectById(id);
        if(yonghuEntity == null)
            return R.error("查不到用户");

        Double yue = yonghuEntity.getNewMoney() - 200;
        if(yue<0)
            return R.error("当前账户不够续费,请联系用户充值");

        yonghuEntity.setNewMoney(yue);
        if(yonghuEntity.getHuiyuanTime().getTime() < new Date().getTime()){
            yonghuEntity.setHuiyuanTime(new Date());
        }

        Calendar instance = Calendar.getInstance();
        instance.setTime(yonghuEntity.getHuiyuanTime());
        instance.add(Calendar.MONTH,1);

        yonghuEntity.setHuiyuanTime(instance.getTime());
        yonghuEntity.setShifouhuiyuanTypes(1);
