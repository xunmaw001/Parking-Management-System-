                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 shifouhuiyuanTypesDiv">
                                        <label>是否会员</label>
                                        <select style="width: 450px" id="shifouhuiyuanTypesSelect" name="shifouhuiyuanTypes" class="form-control">
                                        </select>
                                    </div>
                                    <div class="form-group col-md-6 huiyuanTimeDiv">
                                        <label>会员截止日期</label>
                                        <input style="width: 450px" id="huiyuanTime-input" name="huiyuanTime" type="text" class="form-control layui-input">
                                    </div>
...
<script>
    ...
    // 查询列表
        ...
        function shifouhuiyuanTypesSelect() {
            //填充下拉框选项
            http("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shifouhuiyuan_types", "GET", {}, (res) => {
                if(res.code == 0){
                    shifouhuiyuanTypesOptions = res.data.list;
                }
            });
        }
        <!-- 查询级联表的所有列表 -->
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationShifouhuiyuantypesSelect(){
            var shifouhuiyuanTypesSelect = document.getElementById('shifouhuiyuanTypesSelect');
            if(shifouhuiyuanTypesSelect != null && shifouhuiyuanTypesOptions != null  && shifouhuiyuanTypesOptions.length > 0 ){
                for (var i = 0; i < shifouhuiyuanTypesOptions.length; i++) {
                    shifouhuiyuanTypesSelect.add(new Option(shifouhuiyuanTypesOptions[i].indexName,shifouhuiyuanTypesOptions[i].codeIndex));
                }
            }
        }
    ...
    function setSelectOption() {
        ...
        if(shifouhuiyuanTypesSelect != null && shifouhuiyuanTypesOptions != null  && shifouhuiyuanTypesOptions.length > 0 ) {
            for (var i = 0; i < shifouhuiyuanTypesOptions.length; i++) {
                if (shifouhuiyuanTypesOptions[i].codeIndex == ruleForm.shifouhuiyuanTypes) {//下拉框value对比,如果一致就赋值汉字
                        shifouhuiyuanTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                // newMoney: "required",
                shifouhuiyuanTypes: "required",
                huiyuanTime: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                ...
                // newMoney: "余额不能为空",
                shifouhuiyuanTypes: "是否会员不能为空",
                huiyuanTime: "会员截止日期不能为空",
            }
        }).form();
    }
    ...
    function dateTimePick() {
        var huiyuanTime = laydate.render({
            elem: '#huiyuanTime-input'
            ,type: 'datetime'
        });
    }
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#huiyuanTime-input").val(ruleForm.huiyuanTime);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationShifouhuiyuantypesSelect();
            <!--  初始化级联表的下拉框  -->
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $('#shifouhuiyuanTypesSelect').attr('style', 'pointer-events:none;width:450px;');
            $('#shifouhuiyuanTypesSelect').attr('readonly', 'readonly');
            $('#huiyuanTime-input').attr('readonly', 'readonly');
